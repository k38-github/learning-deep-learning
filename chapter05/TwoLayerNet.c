...
int init(TwoLayerNet *this, int input_size, int hidden_size, int output_size, int batch_size, double weight_init_std) {
    ...
    // Layers
    affinelayer_init(AffineLayer &this->layers.Affine1, this->W1, this->b1, input_size, hidden_size);
    relulayer_init(ReluLayer &this->layers.Relu1, batch_size * hidden_size);
    affinelayer_init(AffineLayer &this->layers.Affine2, this->W2, this->b2, hidden_size, output_size);
    softmaxwithlosslayer_init(SoftmaxWithLossLayer &this->layers.SoftmaxWithLoss, batch_size, output_size);
...
int loss(TwoLayerNet *this, double *ret, double *x, double *t) {
    ...
    softmaxwithlosslaeyr_forward(&this, ret, y, t);
